...
- **AC-A14-1** — `LgCommand.run` asserts, via the shared `assertNoOngoingGroupTraffic(groupNostrIdHex, sinceBeforeLeave)` helper, that no *ongoing* kind-445 group traffic reaches the subscriber's relay connection after the leave settles. The helper opens an `NdkSubscriber` on `[RELAY_URL]`, subscribes to `{ kinds: [445], "#h": [groupNostrIdHex] }`, and closes it in a `finally` block. **The departing member's own RFC 9420 §12.4 Remove proposal — published as a kind-445 on the group `#h` as part of leaving — is excluded from the window** (it is the action, not subsequent traffic). The helper does this by capturing the departure's own event(s) first (filtering from `sinceBeforeLeave`, a timestamp taken *before* the leave click so the relay replays them), then anchoring a 2-second negative-delivery window strictly after the latest departure event (`+1` second, since nostr `created_at` is whole-second granular). Asserts the post-anchor count is `0`. (Prior to this correction the check used a bare `waitForDuration(filter, 2000)` with `since = now` set *after* the click, which raced the whole-second boundary and intermittently counted the leaver's own proposal — counterexample `[A.Cg, A.In(B), A.Lg]`, seed 1508688479.)
- **AC-A14-2** — `FdCommand.run` performs the same `assertNoOngoingGroupTraffic` check in the `leafCount === 1` (last-leaf) branch only, capturing `sinceBeforeForget` before the forget commit so A's own Remove **commit** is likewise excluded. The `leafCount > 1` branch retains its existing A10 check and does not subscribe.
...
- **AC-A14-8** — Interpretation note: A14 asserts wire-level delivery (no *ongoing* kind-445 events arrive at the subscriber's relay connection in the 2-second window that opens after the departure settles), not MLS decryption. **The departure action itself emits exactly one kind-445 on the group `#h` — a Remove proposal for self-leave (RFC 9420 §12.4 forbids self-committing a Remove, so the leaver publishes a proposal that lingers uncommitted in a 2-party group) or a Remove commit when forgetting another member's last leaf. This own-departure event is the action, not in-group traffic the departing party should no longer see, and is therefore excluded from the count** (see AC-A14-1 for the mechanism). The semantics depend on the ephemeral e2e relay honouring `#h` tag filtering and on the strfry container not forwarding kind-445 events to a connection whose subscription filter does not match. This is the operational meaning of "decryptable" in AC-FS-11 for this epic: the leaving user's relay connection no longer sees in-group traffic. If a future epic needs the stronger decryption-attempt assertion, it can extend the fixture.
