...
4. **Formal FSM transition table required before Phase 5 stories.** Nine states are named (`uninitialized` / `joining` / `recovering` / `catching_up` / `buffering_live` / `live` / `degraded` / `retrying_deferred` / `stopped`) but legal transitions, entry/exit conditions, guard predicates, and the `degraded`-as-orthogonal-health encoding are unspecified. Three independent story authors implementing `receive-engine.ts` will produce incompatible state machines without a transition table. `degraded` MUST be modeled as an orthogonal health dimension (e.g., a tagged `{ state, health }` pair), not a flat enum peer of `catching_up`.
...
6. **No formal FSM transition table:** Nine states named but legal transitions, guard predicates, and `degraded`-as-orthogonal-health encoding are unspecified. Required before Phase 5. Three independent story authors without a transition table will produce incompatible state machines. Must be authored by the integration architect and committed to this file or a linked `src/engine/fsm.md` before Phase 5 stories are dispatched.
