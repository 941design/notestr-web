...
**AC-BOUND-3** (Boundary Rule 8 / Implementation Constraint 8) — No file other than `src/engine/engine-types.ts` MUST introduce a new IndexedDB key literal for this epic's stores (`raw-facts`, `accepted-events`, `engine-checkpoints`, `deferred-ids`, `outbox`); persistence implementations MUST reference the exported key constants rather than inlining the string. **(amended 2026-07-12, Stage-2 cold review — P2-8; widened 2026-07-14, S13, to include the S10 `outbox` key, which existed in `engine-types.ts` from S10 onward but was not added to the scanner until S13)**
*Observable:* a grep-based test scans ALL of `src/` (recursive, `.ts` and `.tsx`), excluding only `engine-types.ts` itself, for inlined `notestr:raw-facts:`/`notestr:accepted-events:`/`notestr:engine-checkpoints:`/`notestr:deferred-ids:`/`notestr:outbox:` string/template literals and asserts zero matches; introducing an inlined duplicate anywhere else flips the test to fail. **(amended 2026-07-12, Stage-2 cold review; widened 2026-07-14, S13)**
...
> **Amendment (2026-07-14, S9 review — dual-model ruling, Opus Stage-1 + Fable Stage-2 both concurring).** The observable's "zero added control-flow-branch lines" is **exempt, by role, for the single strangler read-path toggle** introduced by S9: specifically (a) the flag source-selector (`activeRead = isEngineReadPathEnabled() ? {engine source} : {legacy source}` — one ternary feeding the *same* single downstream consumer, not a duplicated branch tree), and (b) its readiness/permission gate boolean composition (`readyForEngine`, `shouldMountEngine = isEngineReadPathEnabled() && readyForEngine`) that conditionally mounts the engine bridge. **Rationale:** a genuine runtime toggle (required by the story's own VQ-S9-001/006) cannot select between two data sources without ≥1 selector branch, and `task-store.tsx` was diff-identical to the Phase-1 baseline before S9, so there was no branch to relocate/offset — net-≤0 is unachievable for any real strangler flag. The carve-out is **scoped by role, not by count** (so the S9-1 blocker fix, which adds a flag term to the mount-gate composition, still complies), and is **pinned to S12's removal obligation**: S12 deletes the flag, the selector, and the gate as the strangler scaffolding is retired, at which point AC-MIG-2's original zero-net-branch observable applies in full again. Boundary Rule 5's actual intent — **no new task-state correctness/CRDT/reducer decision logic** — remains enforced unchanged and was verified clean for S9 by both review stages.
>
> **Closure note (2026-07-14, S13).** In practice S12 removed the legacy listener/branch this carve-out was measured against, but left `isEngineReadPathEnabled()` and the env var itself as unreferenced dead code (by design — see S12's architecture.json), deferring physical deletion to S13. S13 deleted the function, its call sites (`shouldMountEngine` had already stopped consulting it before S13), and the env var reference. This carve-out is now fully closed: `task-store.tsx` carries no strangler selector or flag-gated branch of any kind, and AC-MIG-2's original zero-net-branch observable applies in full going forward.
...
### Optimistic Local Echo (AC-OPT)

*(added 2026-07-14 — closes a gap discovered during S12 spec-validation: the engine's projection is fed only by `group.ingest()`, and MLS deliberately never decrypts a device's own outgoing message back to it (own-echo → `skipped`, never `domain_event_accepted`). The legacy path made a device's own edits visible + durable via task-store's optimistic `applyEvent`/`setState` + legacy `appendEvent` — the exact machinery S12 deletes. spec.md's Layer 4 always required "optimistic local publish intent... reconciliation against durable receive truth"; S10 explicitly deferred the projection-feeding half. This story (S11B, sequenced before S12) closes it so the cutover does not regress own-edit visibility.)*

**AC-OPT-1** — A task the local device dispatches MUST appear in the engine's projected task state **immediately** — before any relay round-trip and with no remote message required — via the **engine boundary** (a locally-accepted domain event fed through the engine's accept path), NOT via an ad-hoc task-store overlay that bypasses the engine.
*Observable:* a test dispatches a `task.created`/`task.updated` through the engine-backed dispatch path with NO remote/`group.ingest` message delivered, and asserts the engine's projection (as exposed by `react-engine-hooks`) contains the task synchronously/eventually-without-remote-input. A test that only asserts a task-store-local `useState` overlay (not the engine projection) does NOT satisfy this — the optimistic value must live in the engine's projection so it survives AC-OPT-2.

**AC-OPT-2** — The optimistically-reflected own task MUST be **durable**: it survives an engine restart (present in the projection rebuilt from the durable accepted-log via `buildProjection(replayOrder(acceptedLog))`), exactly as a remotely-received task would.
*Observable:* a test dispatches a task, restarts the engine (`start({origin:"restored"})`), and asserts the recovered projection still contains the task with the same content — proving the local accept was persisted to the accepted-log, not held only in volatile React state.

**AC-OPT-3** — The eventual relay round-trip of the same own task (marmot own-echo `skipped`, and/or any later delivery of the same content) MUST NOT create a duplicate: the local accept and any echo share the **same deterministic `AcceptedDomainEvent.id`** (derived from the same rumor the outbox sent), and `appendAcceptedEvent` idempotency guarantees the projection reflects the task **exactly once**. The final projected state MUST equal what a pure remote-only flow (no optimistic accept) would produce for the same edit — convergence preserved.
*Observable:* a test dispatches a task (local accept), then feeds the own-echo `skipped` signal for the same task AND a same-id remote delivery, and asserts the projection has exactly one entry for that task with the correct final content; a companion assertion compares the optimistic-then-echo projection against a remote-only projection of the same edit and asserts deep-equality.

**AC-OPT-4** — The projected state MUST let the UI distinguish a **not-yet-confirmed** local edit from a confirmed one (a pending-sync signal — e.g. derived from the `OutboxEntry` status / own-echo-observed flag), and the pending signal MUST clear once the edit is confirmed (own-echo reconciled / durably delivered).
*Observable:* a test dispatches a task, asserts a pending/unconfirmed signal is observable for it; simulates the outbox reconciliation / own-echo, and asserts the signal transitions to confirmed. (The exact surface — a per-task pending flag, or a derivable join against outbox status — is the implementer's choice; the observable is that pending-vs-confirmed is distinguishable and self-clears.)

**AC-OPT-5** (protocol/boundary guard) — Optimistic local echo MUST introduce NO wire-format or convergence-rule change: the local-accept id derivation is deterministic and consistent with the rumor the outbox already sends; `taskWinsOver`/`TaskEvent`/the rumor bytes are untouched; and task-store `dispatch`'s optimistic path routes through the engine boundary so S12 can delete the legacy optimistic `state`/`appendEvent` machinery with no behavioral regression.
*Observable:* a grep/structural check plus a convergence test: the deterministic-id function is the same one the outbox uses; no new rumor field or reducer branch is introduced; and the AC-OPT-3 convergence deep-equality holds.
