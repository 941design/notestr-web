...
- **2026-07-14 (S9 review, dual-model Opus+Fable):** AC-MIG-2 amended with a **role-based carve-out** exempting the single strangler read-path toggle (the flag source-selector ternary + its readiness/mount gate) from the "zero added control-flow-branch" observable — a genuine runtime toggle inherently needs ≥1 selector branch and `task-store.tsx` had no branch to offset it; carve-out pinned to S12's removal of the strangler scaffolding. Boundary Rule 5's no-new-task-state-logic intent remains enforced (verified clean). **S12 prerequisite recorded (from S9 Fable sev-6):** when the engine read-path flag is ON, the engine's marmot-adapter drives `group.ingest()` concurrently with `device-sync.ts`'s still-active group ingest on the *same* MarmotGroup (two uncoordinated pipelines) — so flag-ON is a dev/test-only toggle and **S12 MUST retire/gate `device-sync.ts`'s group ingest driver (device-sync.ts:908-971, group.ingest at :701), not only the applicationMessage listeners, so the engine becomes the SOLE ingest driver.** Otherwise the cutover leaves a dual-ingest hazard.
- **2026-07-14 (S12 spec-validation — cutover gap discovered, new story S11B inserted):** The integration-architect's Phase-1 validation of S12 (the legacy→engine cutover) surfaced a real, user-authorized-decision-worthy gap: the engine's projection is fed exclusively by `group.ingest()`, and MLS deliberately suppresses own-echo (a device never decrypts its own outgoing message back — own-echo maps to `skipped`, never `domain_event_accepted`). The legacy path made a device's OWN edits visible + durable via task-store's optimistic `applyEvent`/`setState` + legacy `appendEvent` — exactly the machinery S12 removes. Layer 4 of this spec always required "optimistic local publish intent... reconciliation against durable receive truth," but S10 explicitly scoped in only the durable-send/outbox-status half and left the projection-feeding optimistic half in task-store (React-owned). Removing it in S12 without a replacement would make a user's own just-created/edited task vanish from their screen — a severe regression. **Resolution (user decision 2026-07-14): build the missing optimistic-local-echo mechanism at the engine boundary as new story S11B, sequenced BEFORE S12, so a locally-dispatched task appears in the engine projection immediately (marked pending until confirmed), persists durably (accepted-log), and reconciles/dedupes by its deterministic rumor id — then proceed with the S12/S13 cutover.** New acceptance criteria AC-OPT-1..5 added. No wire-format/convergence change (AC-OPT-5 guard).
